...
export default function ProjectsPage() {
  const actionClass =
    "btn-retro inline-flex min-w-[92px] items-center justify-center border-2 border-[#0f0f0f] bg-[#f6eddc] px-3 py-1.5 text-[11px] uppercase tracking-wide text-[#2f2519] [box-shadow:2px_2px_0_#0f0f0f] hover:bg-[#e6d8b8] dark:border-[#1a2240] dark:bg-[#3a4870] dark:text-[#dee2ec] dark:[box-shadow:2px_2px_0_#1a2240] dark:hover:bg-[#445278] sm:text-[12px]";
  ...
  return (
    ...
    >
      <motion.article
        ...
        className={`${readableFont.className} border-4 border-[#0f0f0f] bg-[#f7efdf] p-4 font-bold [box-shadow:4px_4px_0_#0f0f0f] dark:border-[#1a2240] dark:bg-[#2e3c60] dark:[box-shadow:4px_4px_0_#1a2240]`}
      >
          ...
          {projects.map((project, index) => {
            ...
            return (
              <motion.div
                ...
                className="border-3 border-[#0f0f0f] bg-[#fff9eb] p-3 text-[13px] text-[#2f281f] [box-shadow:5px_5px_0_#0f0f0f] transition-[box-shadow] duration-150 hover:-translate-y-0.5 hover:[box-shadow:6px_7px_0_#0f0f0f] dark:border-[#1a2240] dark:bg-[#324268] dark:text-[#acb2c8] dark:[box-shadow:5px_5px_0_#1a2240] dark:hover:[box-shadow:6px_7px_0_#1a2240] sm:p-4 sm:text-[15px]"
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
                  <div className="relative aspect-[16/10] w-full shrink-0 overflow-hidden border-3 border-[#0f0f0f] bg-white dark:border-[#1a2240] dark:bg-[#2e3c60] sm:w-[20rem] md:w-[24rem]">
                    {project.image ? (
                      <Image
                        src={project.image}
                  ...
                  <div className="flex min-w-0 flex-1 flex-col">
                    <p className="text-[20px] text-[#3a3126] dark:text-[#eceef8] sm:text-[24px]">
                      {project.name}
                    </p>
                    <ul className="mt-2 space-y-1.5 text-[12px] leading-relaxed sm:text-[14px]">
                      {project.bullets.map((bullet, i) => (
                        <li key={i} className="flex gap-2">
                          <span className="mt-1.5 h-1.5 w-1.5 shrink-0 border border-[#0f0f0f] bg-[#d4c9b0] dark:border-[#1a2240] dark:bg-[#626a98]" />
                          <span>{bullet}</span>
                        </li>
                      ))}
                    </ul>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {showWebsiteButton ? (
                        hasProjectLink ? (
                          ...
                          <span className="inline-flex min-w-[92px] items-center justify-center border-2 border-[#0f0f0f] bg-[#f3ead5] px-3 py-1.5 text-[11px] uppercase tracking-wide text-[#6e614d] [box-shadow:2px_2px_0_#0f0f0f] dark:border-[#1a2240] dark:bg-[#2e3c60] dark:text-[#6a7295] dark:[box-shadow:2px_2px_0_#1a2240] sm:text-[12px]">
                            <span className="mr-1.5 inline-flex h-4 w-4 items-center justify-center overflow-hidden">
                              <Image
                                src="/globe.svg"
                                alt="Website icon"
